...
class Heap {
    ...
    public:
        ...
        void insert(T element) {
            ...
            heapIndex i = tree.size() - 1;
            
            while (i > 1) {
                if (tree[getParentPosition(i)] > tree[i]) {
                    std::swap(tree[i], tree[getParentPosition(i)]);
        ...
        void remove(T value) {
            heapIndex bottomrightvalue = tree.size() - 1;
            for (heapIndex i = 1; i < tree.size(); i++) {
                    if (tree[i] == value) {
                        std::swap(tree[i], tree[bottomrightvalue]);
                        tree.pop_back();
                        heapifyDown(i);
                        break;
                    }
